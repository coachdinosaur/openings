...
test("publishes all Markdown chapters through one learner route", async () => {
  const responses = await Promise.all([render("/"), render("/chapters/1"), render("/chapters/9"), render("/chapters/11"), render("/chapters/12"), render("/chapters/13"), render("/chapters/14"), render("/chapters/15")]);
  ...
  assert.match(await responses[7].text(), /aria-label="Chapter page 1, 1 of 24"/, "Chapter 15 should open on its variation index");
  assert.equal((await render("/chapters/16")).status, 404);
...
test("all fifteen Markdown source files and core chess assets are packaged", async () => {
  const chapterFiles = (await readdir(new URL("app/content/chapters/", root))).filter((name) => /^chapter-\d+-catalan\.md$/.test(name));
  assert.equal(chapterFiles.length, 15);
  await Promise.all([
