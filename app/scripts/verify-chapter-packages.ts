...
async function verifyPackage(id: number) {
  ...
  if (!legacy && config.manifest.lesson.expectedCanonicalHash && canonicalHash !== config.manifest.lesson.expectedCanonicalHash) errors.push(`canonical lesson hash ${canonicalHash} does not match manifest`);
  let evidenceInventoryHash = "";
  if (!legacy) evidenceInventoryHash = await verifyEvidence(config, errors);
