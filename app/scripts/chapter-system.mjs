...
const routingPath = path.join(appRoot, "app", "chapter-routing.generated.ts");
...
function routingSource(packages) {
...
async function syncRegistry({ checkOnly = false } = {}) {
  ...
  const expectedRouting = routingSource(packages);
  const currentRouting = await readFile(routingPath, "utf8").catch(() => "");
  if (currentRouting !== expectedRouting) {
    if (checkOnly) fail("The generated chapter routing catalog is stale. Run `npm run chapters:sync`.");
    await writeFile(routingPath, expectedRouting, "utf8");
    console.log(`Updated ${path.relative(appRoot, routingPath)} for ${packages.length} chapters.`);
  } else if (!checkOnly) {
    console.log(`Chapter routing catalog is already current (${packages.length} chapters).`);
  }
  return packages;
}
