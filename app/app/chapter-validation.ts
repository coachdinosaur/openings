...
export function validateChapterConfig(config: ChapterConfig): string[] {
  ...
  if (!legacy && lesson.source.sha256 !== config.importDefinition.sourceHash) errors.push("lesson and package source hashes differ");
  if (!legacy && !/^[A-F0-9]{64}$/.test(lesson.source.sha256)) errors.push("source SHA-256 must be 64 uppercase hexadecimal characters");
  if (!legacy && config.id !== "1" && !/^[A-F0-9]{64}$/.test(config.importDefinition.expectedCanonicalHash ?? "")) errors.push("canonical lesson hash is missing or invalid");
  ...
  if (!legacy && lesson.diagrams.length !== config.manifest.evidence.diagramCount) errors.push(`expected ${config.manifest.evidence.diagramCount} diagrams, found ${lesson.diagrams.length}`);
