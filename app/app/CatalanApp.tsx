...
function LessonView({ config, overlay }: { config: ChapterConfig; overlay: ReviewOverlay }) {
  ...
  return <main className="view lesson-reader">
    <section className="lesson-heading"><div><p className="eyebrow">{config.label} · Complete guided source lesson</p><h1>{lesson.title}</h1><p className="lede">Read the complete chapter in verified two-column order. Select a highlighted move to synchronize the board.</p></div><span className="status-pill verified">✓ Source text verified</span></section>
...
export default function CatalanApp({ chapterId = "1", initialView = "lesson" }: { chapterId?: "1" | "2"; initialView?: View }) {
  ...
  return <div className="app-shell">
    <header className="topbar"><button className="menu-button" onClick={() => setMenuOpen((value) => !value)} aria-label="Toggle navigation">☰</button><button className="brand" onClick={() => navigate("lesson")}><span className="brand-mark">C</span><span><strong>Catalan Atelier</strong><small>{config.label}</small></span></button><div className="top-source"><span>Reading order</span><strong>{config.printedRange} · left → right</strong></div></header>
    <aside className={`sidebar ${menuOpen ? "open" : ""}`}><nav><p>Course</p><a className="course-link" href="/chapters/1"><span className="nav-glyph">1</span><span>Chapter 1 lesson<small>Complete · A through C</small></span></a><a className="course-link" href="/chapters/2"><span className="nav-glyph">2</span><span>Chapter 2 lesson<small>Complete · A through C2</small></span></a>{editorMode && <><p className="editor-nav-label">Editor tools</p><a className={view === "review" ? "active" : ""} href={`/chapters/${config.id}/review`}><span className="nav-glyph">R</span><span>Source review<small>Text, moves & boards</small></span></a><a className={view === "import" ? "active" : ""} href={`/chapters/${config.id}/import`}><span className="nav-glyph">I</span><span>Re-import lesson<small>{config.importDefinition.scopeLabel}</small></span></a></>}</nav><div className="sidebar-footer"><div className="source-card"><span>Verified source</span><strong>{config.importDefinition.filename}</strong><small>SHA-256 · {config.importDefinition.sourceHash.slice(0, 4)}...{config.importDefinition.sourceHash.slice(-4)}</small></div><button className={`editor-toggle ${editorMode ? "on" : ""}`} onClick={toggleEditor} role="switch" aria-checked={editorMode}><span className="toggle-track"><i /></span><span><strong>Editor mode</strong><small>{editorMode ? "Review tools visible" : "Learner view only"}</small></span></button><p className="local-only">Local only · no data leaves this device</p></div></aside>
    {menuOpen && <button className="scrim" onClick={() => setMenuOpen(false)} aria-label="Close navigation" />}
    <div className="chapter-switcher" aria-label="Chapters"><a className={config.id === "1" ? "active" : ""} href="/chapters/1">Chapter 1</a><a className={config.id === "2" ? "active" : ""} href="/chapters/2">Chapter 2</a></div>
