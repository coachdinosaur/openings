...
1. Run `npm run chapters:status` and `npm run chapters:check` from `app/`. Stop and fix catalog/package drift first.
...
6. Implement the complete `app/app/chapterN-lesson.ts`, source page images/crops, `app/app/chapter-manifests/chapter-N.json`, and `app/app/chapter-packages/chapter-N.ts`. Keep canonical SAN separate from printed annotations and derive legal FENs with `chess.js`. Run `npm run chapters:validate-lesson -- app/chapterN-lesson.ts` repeatedly while transcribing; it reports the first illegal source ply and the canonical lesson hash.
7. Freeze the reported canonical hash and source/evidence facts in the versioned manifest. Run `npm run chapters:verify -- --chapter N`, then `npm run chapters:sync`. Synchronization imports and verifies packages before the generated catalog supplies navigation, loading, and all shared routes; do not edit it.
8. Run `npm run chapters:verify-import -- app/chapterN-lesson.ts app/chapter-packages/chapter-N.ts ..\ChapterN_Catalan.pdf`, then `npm run chapters:check`, `npm test`, and `npm run lint`. Inspect `/chapters/N`, `/chapters/N/review`, and `/chapters/N/import` at desktop and narrow widths. The import URL is a compatibility route for source verification; it must not claim to reconstruct the lesson.
